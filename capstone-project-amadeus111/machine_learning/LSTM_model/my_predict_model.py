...
def required_processing(question,answers):  #question数量为1 ，answers是top5 的list
    path=os.path.dirname(os.path.dirname(os.path.abspath('my_predict_model.py')))+'/machine_learning/LSTM_model/data/embedding'
    ...
    for i in range(len(answers)):
        question_matrix.append(get_word_matrix(question,embedding_matrix))
    for each_answer in answers:
        answer_matrix.append(get_word_matrix(each_answer,embedding_matrix))
...
def run_predict(question,answers):
    ...
    print(os.path.dirname(os.path.dirname(os.path.abspath('my_predict_model.py')))+'/machine_learning/LSTM_model/LSTMmodel_alltrain.h5')
    model = load_model(os.path.dirname(os.path.dirname(os.path.abspath('my_predict_model.py')))+'/machine_learning/LSTM_model/LSTMmodel_alltrain.h5', custom_objects={'AttentionWithContext': AttentionWithContext})
