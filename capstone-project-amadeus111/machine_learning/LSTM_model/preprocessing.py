...
def connectdb():
    ...
    db = pymysql.connect("127.0.0.1","root","666666" ,"comp9900")   #my local database,should change to aws online database
    print('连接上了!')
    return db
...
def test_csv_data(raw_data,index):#  正确答案为anser【0】
    ...
    for que_index in range(len(question_list)):
        if que_index<len(question_list):
            answer = np.random.choice(np.hstack((answer_list[:que_index] , answer_list[que_index+1:])),5)
        else:
            answer = np.random.choice(answer_list[:que_index],5)
        answer = np.append(answer,answer_list[que_index])
        # random.shuffle(answer)
        wrong_match.append([question_list[que_index],answer[0],answer[1],answer[2],answer[3],answer[4],answer[5]])
    df = pd.DataFrame(wrong_match, columns=['question', 'distractor0','distractor1','distractor2','distractor3','distractor4','distractor5'])
    print(df)
...
def cal_matrix_test(file_index):
    ...
    for index in range(0, len(file)):
        ...
        question_words1 = get_word_matrix(each_question, embedding_matrix)
        answer_words0 = get_word_matrix(each_answer0, embedding_matrix)
        answer_words1 = get_word_matrix(each_answer1, embedding_matrix)
        answer_words2 = get_word_matrix(each_answer2, embedding_matrix)
        answer_words3 = get_word_matrix(each_answer3, embedding_matrix)
        answer_words4 = get_word_matrix(each_answer4, embedding_matrix)
        answer_words5 = get_word_matrix(each_answer5, embedding_matrix)
        for i in range(6):
...
def get_word_matrix(string,embedding):
    word_list = string.split(' ')
    centence_matrix = []
    padding = 120 - len(word_list)
    if padding >= 0 :
        for word in word_list:
            try:
                centence_matrix.append(embedding[word])
            except:
                centence_matrix.append(np.random.normal(loc=0.0, scale=1.0, size=100))
...
def cal_matrix_train(file_index):
    ...
    for index in range(0,len(file)):
        ...
        question_words1 = get_word_matrix(each_question,embedding_matrix)
        answer_words1 = get_word_matrix(each_answer,embedding_matrix)
        question_matrix.append(question_words1)
        answer_matrix.append(answer_words1)
        # break
